...
import { CommentRepositoryInterface, CreateCommentInterface } from "../../../../domain/repositories/comment_repository";
...
export class CommentRepository implements CommentRepositoryInterface{
    async create(comment: CreateCommentInterface): Promise<Comment> {
        ...
        if(comment.comment_id){
            const existingComment = await CommentEntity.findOneBy({id: comment.comment_id})
            if(existingComment)
                newComment.comment = existingComment
        } 
